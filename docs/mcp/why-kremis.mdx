...
2. **Visible state**. Every node and edge came from a signal you sent. There are no hidden inferences. Stage classification (`S0`–`S3`) is deterministic and based on edge-confidence thresholds defined in `crates/kremis-core/src/system/stage.rs`.
