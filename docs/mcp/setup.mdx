---
title: "MCP Setup"
description: "How to configure the Kremis MCP server with AI assistants."
icon: "gear"
---

## Prerequisites

1. A running Kremis HTTP server
2. The `kremis-mcp` binary (built from `apps/kremis-mcp` or extracted from a release tarball)

## Build

```bash
cargo build -p kremis-mcp --release
```

## Configuration

| Variable | Default | Description |
|----------|---------|-------------|
| `KREMIS_URL` | `http://127.0.0.1:8080` | Kremis server URL |
| `KREMIS_API_KEY` | (none) | Optional Bearer token |
| `KREMIS_LOG_FORMAT` | `text` | Log output format: `text` or `json` |

<Note>
  Prefer `127.0.0.1` over `localhost`. The server binds `127.0.0.1` by default; on Windows `localhost` resolves to `::1` first, and each new connection waits for the IPv6 attempt to fail before falling back.
</Note>

## Claude Desktop

Add to your `claude_desktop_config.json`:

```json
{
  "mcpServers": {
    "kremis": {
      "command": "/path/to/kremis-mcp",
      "env": {
        "KREMIS_URL": "http://127.0.0.1:8080",
        "KREMIS_API_KEY": "your-key-here"
      }
    }
  }
}
```

## Claude Code

Add to your `.mcp.json` or project settings:

```json
{
  "mcpServers": {
    "kremis": {
      "command": "/path/to/kremis-mcp",
      "env": {
        "KREMIS_URL": "http://127.0.0.1:8080"
      }
    }
  }
}
```

## Manual Testing

```bash
# Start the Kremis server first
kremis server

# In another terminal, run the MCP server
KREMIS_URL=http://127.0.0.1:8080 ./target/release/kremis-mcp
```

The MCP server communicates via stdin/stdout using JSON-RPC. It will wait for MCP protocol messages.
