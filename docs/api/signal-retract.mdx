...
weights are architecturally forbidden because `strongest_path` uses Dijkstra
with `cost = -weight`, and the algorithm requires non-negative costs after
negation (i.e., weights must be ≥ 0).
