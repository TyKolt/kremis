...
[mcp]
url = "http://127.0.0.1:8080"   # Kremis server URL used by the MCP bridge
```
...
| `KREMIS_URL` | `[mcp] url` | `http://127.0.0.1:8080` |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `url` | string | `"http://127.0.0.1:8080"` | URL of the Kremis HTTP server. Used by `kremis-mcp` when proxying tool calls. |
