...
<Warning>
  **Status: design specification.** This document freezes the certificate
  contract so an independent implementation can reproduce it. The format is
  versioned; changes require a version increment and updated test vectors.
</Warning>
...
| `evidence_subgraph` | nodes + edges | Minimal dependency set, `BTreeMap`-ordered |
