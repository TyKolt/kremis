...
<Warning>
  **Status: design specification.** The Verifiable Query Certificate is under
  implementation. This page defines the contract; it does not describe a
  shipped feature. Sections marked _Implemented_ reference existing behavior.
</Warning>
...
**Verifiable Query Certificate (VQC)** — a self-contained object that a third
party re-executes to obtain the identical answer, offline, without trusting
...
| `evidence_subgraph` | The minimal set of nodes and edges the result depends on |
...
- _Implemented._ All core collections use `BTreeMap` — iteration order is
  deterministic, no `HashMap` anywhere in `kremis-core`.
- _Implemented._ The canonical export (`KREX`, magic `b"KREX"`, version 2,
  postcard) is bit-exact; the same state serializes to the same bytes.
- _Implemented._ `GET /hash` already returns a BLAKE3 hash over that
  canonical form.
